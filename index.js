...
app.post('/submit', async (request, response) => {
  ...
  try {
    const orderdeskRequest = await fetch(`https://app.orderdesk.me/api/v2/orders?source_id=${orderId}`, {
      method: "GET",
      headers: {
    ...
    const orderdeskResponse = await orderdeskRequest.json(); // Wait for JSON parsing

    console.log(orderdeskResponse);

    response.status(200).json({
      message: 'Order fetched successfully',
      matching_order: orderdeskResponse
    });
  } catch (error) {
